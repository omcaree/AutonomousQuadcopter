...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(Serial_M) == (NULL)) && !rtmGetStopRequested
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(Serial_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(Serial_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(Serial_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.05s, 0.0s] */
      rtExtModeUpload(0, Serial_M->Timing.taskTime0);
    }
...
int main(int argc, char **argv)
{
  ...
  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(Serial_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(Serial_M, true);
    }
  }
