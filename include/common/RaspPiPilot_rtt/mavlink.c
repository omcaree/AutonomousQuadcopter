...
//     fprintf(stderr,"itteration i: %i , length: %i \n",ii,ll);
    fprintf(stderr,"msg id : %i \t %i \n",msg.msgid,msg.seq);
//     fprintf(stderr,"msg id : %i \n",msg.msgid);
    return msg.msgid;
}
...
fprintf(stderr,"HB sys status : %i \n",heartMsg.system_status);
}
...
uint8_t sendArmDisarm(uint8_t* buf, uint8_t armdisarm){
    ...
    mavlink_msg_command_long_pack(255,0,&msgOut,1,250,400,255,
