...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(RaspPiPilot_M) == (NULL)) && !rtmGetStopRequested
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(RaspPiPilot_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(RaspPiPilot_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(RaspPiPilot_M, true);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(RaspPiPilot_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(RaspPiPilot_M, true);
    }
  }
